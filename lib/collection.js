...
module.exports = function Collection(db, state) {
  ...
  var interface = {
    ...
    updateOne: function (selector, data, options, callback) {
      ...
      var result = {connection: db, n: 0, u: 0};
      var action = (options.upsert ? "upsert: " : "updateOne: ");
      debug('%s.%s %j', name, action, selector);

      asyncish(function () {
        var doc = first(selector, state.documents || []) || {};
        debug('%s.%s %j', name, action, doc);
        ...
        if (options.upsert) {
          ...
          if (Object.keys(original).length === 0) result.n = 1;
          else result.u = 1;

          _.merge(original, selector, data.$set);

          if(!state.documents) state.documents = [original];
          else state.documents.push(original);
          ...
          if (Object.keys(original).length > 0) result.u = 1;
